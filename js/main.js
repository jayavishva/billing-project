...
function renderMenu() {
    ...
    if (menu.length === 0) {
        menuGrid.innerHTML = '<p>No menu items available. Please add items from admin panel.</p>';
        return;
    }
    ...
    billDetails.innerHTML = `
        ...
        <ul style="margin-left: 20px; margin-top: 10px;">
            ${billData.items.map(item => 
                `<li>${item.name} x ${item.quantity} = ${formatCurrency(item.subtotal)}</li>`
            ).join('')}
        </ul>
...
function printBill() {
    ...
    document.getElementById('printDate').textContent = `Date: ${formatDateTime(new Date())}`;
    document.getElementById('printTransactionId').textContent = `Transaction ID: ${transactionId}`;
    
    const printItems = document.getElementById('printItems');
    printItems.innerHTML = billData.items.map(item => `
        <div class="bill-item">
